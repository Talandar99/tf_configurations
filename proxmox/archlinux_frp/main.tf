...
variable "vmid" {
  type      = number
  default = 662  # unique container ID
}
...
resource "null_resource" "setup" {
  provisioner "remote-exec" {
    ...
    inline = [
      ...
      "lxc-attach -n ${var.vmid} -- echo 'PermitRootLogin yes' | lxc-attach -n ${var.vmid} -- tee -a /etc/ssh/sshd_config > /dev/null",
      ...
      "lxc-attach -n ${var.vmid} -- ./shellfish/scripts/install_frp_form_source.sh",
      "echo IP---------------------------IP",
